...
ggplot(umsatzdaten) +
  ...
  geom_point(aes(x = wochentag, y = Umsatz), size=0.1) 
...
ggplot(umsatzdaten) +
  facet_wrap(~Warengruppe) +
  geom_point(aes(x = Temperatur, y = Umsatz), size=0.1) 
```
```{r}
#UMSATZ ~ BEWOELKUNG
ggplot(umsatzdaten) +
  facet_wrap(~Warengruppe) +
  geom_point(aes(x = Bewoelkung, y = Umsatz), size=0.1)
```
```{r}
#UMSATZ ~ Holiday
ggplot(umsatzdaten) +
  facet_wrap(~Warengruppe) +
  geom_point(aes(x = holiday, y = Umsatz), size=0.1)
```
...
ggplot(umsatzdaten) +
  facet_wrap(~Warengruppe) +
  geom_point(aes(x = Windgeschwindigkeit, y = Umsatz), size=0.1) 
```
