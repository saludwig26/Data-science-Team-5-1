...
history = model.fit(r.train_dataset, r.train_labels, epochs=15, validation_data = (r.test_dataset, r.test_labels), verbose=0)
...
vorhersage <- as.Date("2019-06-04")

umsatzdaten_norm <- rbind(umsatzdaten_norm, tibble(Datum = rep(vorhersage,6), Warengruppe = (1:6), Umsatz=NA))
```
...
vorhersage_predictions_norm <- py$model$predict(vorhersage)


# Rückberechnung der normierten Umsatzschätzungen zu den tatsächlichen Umsatzschätzungen 
...
vorhersage_predictions <- (vorhersage_predictions_norm * norm_values_list$sd[1]) + norm_values_list$mean[1]
# Selektion der zugehörigen tatsächlichen Umsatz

train_actuals <- umwandlung_umsatzdaten$Umsatz[train_ind]
test_actuals <- umwandlung_umsatzdaten$Umsatz[-train_ind]
vorhersage_actuals <- umwandlung_umsatzdaten$Umsatz[vorhersage]
