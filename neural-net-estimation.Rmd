...
model = keras.Sequential([
  layers.Dense(30, activation='relu', input_shape=[len(r.train_dataset.keys())]),
...
model.save("python_model_new.h5")
...
```{python}
model = keras.models.load_model("python_model_new.h5")
...
```{r}
# Schätzung der (normierten) Preise für die vorhersage date
vorhersage_predictions_norm <- py$model$predict(vorhersage_dataset)
# Rückberechnung der normierten Umsatzschätzungen zu den tatsächlichen Umsatzschätzungen 
vorhersage_predictions <- (vorhersage_predictions_norm * norm_values_list$sd[1]) + norm_values_list$mean[1]
# Selektion der zugehörigen tatsächlichen Umsatz
vorhersage_actuals <- umwandlung_umsatzdaten$Umsatz[NA]
